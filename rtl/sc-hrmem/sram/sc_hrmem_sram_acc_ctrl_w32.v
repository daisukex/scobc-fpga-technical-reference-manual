...
assign w_ren_sel   = REN | r_ren_p1;
assign w_radr_sel  = (REN) ? RADR: r_radr_p1;
assign w_rbten_sel = (REN) ? RBTEN: r_rbten_p1;
...
always@ (posedge CLK or negedge RESET_N) begin
  if (!RESET_N) begin
    ...
    if ((|r_rbten_p1[1:0] & SR1_ERR) | (|r_rbten_p1[3:2] & SR2_ERR)) begin
      ECC1ERR     <= 1'b1;
      ECCERR_ADR  <= r_radr_p1;
      ECCERR_BTEN <= r_rbten_p1;
    end
  end
  else begin
    RDATA       <= 0;
